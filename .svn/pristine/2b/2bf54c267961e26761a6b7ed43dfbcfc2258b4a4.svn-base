...
import com.job5156.core.common.CommonEnum.PositionStatus;
...
public class ComPosition implements Serializable {
    ...
    public String getFirstCityName() {
        String cityName = "";
        Integer firstCityCode = getFirstCityCode();
        if(firstCityCode > 0){
        	cityName = OptionCityMap.isTown(firstCityCode) ? OptionMap.getTownStr(firstCityCode, null) : OptionMap.getCityAddr(firstCityCode);
        }
        return cityName;
    }
    ...
    public String getReqLanguageStr() {
        String str = "";
        if(getReqLanguage() != null && getReqLanguage() > 0){
    ...
    public String getAddressOfProAndCity() {
        String str = "";
        if(getComContactBase() != null && StringUtils.isNotBlank(getComContactBase().
                getAddress()) && !StringUtils.equals("{}",getComContactBase().getAddress())) {
            Map<String,String> map = new Gson().fromJson(getComContactBase().getAddress(), new TypeToken<Map<String,String>>() {}.getType());
    ...
    public String getAddressOfDistr() {
        String str = "";
        if(getComContactBase() != null && StringUtils.isNotBlank(getComContactBase().
         getAddress()) && !StringUtils.equals("{}",getComContactBase().getAddress())) {
            Map<String,String> map = new Gson().fromJson(getComContactBase().getAddress(), new TypeToken<Map<String,String>>() {}.getType());
    ...
    public Integer getMaxSalaryWithTrans() {
        if (getMaxSalary() == null || getMaxSalary().equals(0)) {//当最大薪资无任何数值时进行转换
            if (getSalary() == null || getSalary() <= 0) { //默认为0
                ...
                Integer[] salaryArray = salaryMap.get(getSalary());
                if (salaryArray != null) {
