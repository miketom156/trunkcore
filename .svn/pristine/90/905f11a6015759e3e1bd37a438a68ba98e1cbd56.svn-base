...
public class PosBrief {
    ...
    private transient Integer maxSalary; //最高薪资，只为显示，不入库
    private transient Integer minSalary; //最低薪资，只为显示，不入库
    private transient Integer negotiableFlag; //是否显示为面议   '是否面议，0,Null=否，1=是'
	...
	public String getSrvExtendNo() {
		if (StringUtils.isNotBlank(ObjectUtils.toString(getSrvExtend()))) {
			Map<String, String> extendMap = new Gson().fromJson(getSrvExtend(), new TypeToken<Map<String, String>>() {
			}.getType());
	...
	public ComPosition getComPosition() {
		...
		if (StringUtils.isNotBlank(getJsoninfo())) {
			Map<String, Object> jsonMap = gson.fromJson(getJsoninfo(), new TypeToken<Map<String, Object>>() {
			}.getType());
	...
	public String getSalaryStr() {
		return OptionMap.getSalaryStr(getSalary(), getMinSalary(), getMaxSalary(), getNegotiableFlag());
	}
    ...
    public Integer getMaxSalary(){
        ...
        if(comPosition != null && comPosition.getMaxSalary()!= null){
            return NumberUtils.toInt(comPosition.getMaxSalary()+"",0);
        }
        return 0;
    }
    public Integer getNegotiableFlag(){
        ...
        ComPosition comPosition = getComPosition();
        if(comPosition != null && comPosition.getNegotiableFlag()!= null){
            return NumberUtils.toInt(comPosition.getNegotiableFlag()+"",0);
        }
        return 0;
    }
    public Integer getMinSalary(){
        ...
        ComPosition comPosition = getComPosition();
        if(comPosition != null && comPosition.getMinSalary() != null){
            return NumberUtils.toInt(comPosition.getMinSalary()+"",0);
        }
        return 0;
    }
