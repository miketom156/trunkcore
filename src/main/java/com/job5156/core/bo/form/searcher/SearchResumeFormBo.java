...
//    private Integer onlineTalents;                //1为到场人才
    ...
    private List<Integer> excludeIdList;			//排除简历id列表
    private String keywordQueryStr;					//关键字组合（包含词之间的关系）
    private List<String> posKeywordList;			//职位关键字列表
    ...
    public String getLocationListStr() {
        ...
        if(locationList!=null) {
            for (Integer val:locationList){
                retVal+=(String.valueOf(val)+",");
            }
        }
        if(retVal.endsWith(",")){return retVal.substring(0,retVal.length()-1);}
        return retVal;
	}
	...
	public String getWorkareaListStr() {
        String retVal="";
        if(workareaList!=null) {
            for (Integer val:workareaList){
                retVal+=(String.valueOf(val)+",");
            }
        }
        if(retVal.endsWith(",")){return retVal.substring(0,retVal.length()-1);}
	...
	public String getKeywordQueryStr() {
		return keywordQueryStr;
	}

	public void setKeywordQueryStr(String keywordQueryStr) {
		this.keywordQueryStr = keywordQueryStr;
	}
