...
import com.job5156.core.common.util.KeySynchronizer;
...
public class PageUtil {
    ...
    public static Field getPkField(Class<?> cls) {
        ...
        //递归父类中是否有Id
        if(pkField == null && cls.getSuperclass() != null) {
        	pkField = getPkField(cls.getSuperclass());
        }
    ...
    public static String getPageUrl(String pageURI, String pageQuery) {
    	if(pageQuery != null) {
            pageQuery = pageQuery.replaceAll("&pn=\\d*", "").replaceAll("pn=\\d*", "").replaceAll("&pre=true", "");
