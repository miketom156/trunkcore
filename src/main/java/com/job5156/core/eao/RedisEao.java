...
import com.job5156.core.common.util.SerializeUtil;
...
public class RedisEao {
    ...
    public Boolean isKeyExist(final String key) {
        return getRedisTemplate(key).execute(new RedisCallback<Boolean>() {
    ...
    * @throws
    ...
    public Long getKeyExpireTime(final String key) {
    	return getRedisTemplate(key).execute(new RedisCallback<Long>() {
    		@Override
    ...
    public void writeToString(final String key, final String value) {
        getRedisTemplate(key).execute(new RedisCallback<Object>() {
            @Override
    ...
    public String readFromString(final String key) {
        return getRedisTemplate(key).execute(new RedisCallback<String>() {
            @Override
    ...
    public <T> T readRedisVoFromHash(final String key, final T vo) {
        return getRedisTemplate(key).execute(new RedisCallback<T>() {
            @Override
    ...
    public <T> List<T> readObjectFromList(final String key, final Class<T> cla, final int begin, final int end){
    	return getRedisTemplate(key).execute(new RedisCallback<List<T>>() {
    ...
    public List<String> readFromList(final String key) {
        return getRedisTemplate(key).execute(new RedisCallback<List<String>>() {
    ...
    public Set<String> readFromSet(final String key) {
        return getRedisTemplate(key).execute(new RedisCallback<Set<String>>() {
    ...
    public <T> void writeObjectToMap(final String key, final Map<String, T> map){
    	getRedisTemplate(key).execute(new RedisCallback<Boolean>() {
    ...
    public <T> Map<String, T> readObjectFromMap(final String key, final Class<T> cla){
    	return getRedisTemplate(key).execute(new RedisCallback<Map<String, T>>() {
    ...
    public Boolean removeFromMap(final String key, final String mapKey){
    	return getRedisTemplate(key).execute(new RedisCallback<Boolean>() {
			...
			public Boolean doInRedis(RedisConnection connection) throws DataAccessException {
				byte[] redisKey = getRedisTemplate(key).getStringSerializer().serialize(key);
				byte[] fieldKey = getRedisTemplate(key).getStringSerializer().serialize(mapKey);
				return connection.hDel(redisKey, fieldKey);
			}
		});
    }
    ...
    public Boolean removeFromSet(final String key, final String value) {
        return getRedisTemplate(key).execute(new RedisCallback<Boolean>() {
            ...
            public Boolean doInRedis(RedisConnection connection) throws DataAccessException {
                ...
                byte[] redisValue = getRedisTemplate(key).getStringSerializer().serialize(value) ;
                if(connection.exists(redisKey)) {
                    return connection.sRem(redisKey, redisValue);
                } else {
                    return false;
                }
            }
    ...
    public Long zRank(final String key, final String value){
    	return getRedisTemplate(key).execute(new RedisCallback<Long>() {
			...
			public Long doInRedis(RedisConnection connection) throws DataAccessException {
				byte[] redisKey = getRedisTemplate(key).getStringSerializer().serialize(key);
				byte[] redisVal = getRedisTemplate(key).getStringSerializer().serialize(value);
						
				return connection.zRank(redisKey, redisVal);
			}
    	});
    }
    ...
    public Long hIncrBy(final String key, final String field, final Integer value){
    	return getRedisTemplate(key).execute(new RedisCallback<Long>() {
            ...
            public Long doInRedis(RedisConnection connection) throws DataAccessException {
                byte[] redisKey = getRedisTemplate(key).getStringSerializer().serialize(key);
                byte[] fieldVal = getRedisTemplate(key).getStringSerializer().serialize(field);
                return connection.hIncrBy(redisKey, fieldVal, value);
            }
        });
    }
     ...
     * @return
     */
    public Map<String, Integer> hGetAll(final String key){
    	return getRedisTemplate(key).execute(new RedisCallback<Map<String, Integer>>() {

			@Override
            public Map<String, Integer> doInRedis(RedisConnection connection) throws DataAccessException {
                byte[] redisKey = getRedisTemplate(key).getStringSerializer().serialize(key);
                Map<String, Integer> dataMap = null;
                Map<byte[], byte[]> redisMap = connection.hGetAll(redisKey);
                if(MapUtils.isNotEmpty(redisMap)){
                	dataMap = Maps.newHashMap();
                	for(Map.Entry<byte[], byte[]> entry : redisMap.entrySet()){
                		String fieldKey = getRedisTemplate(key).getStringSerializer().deserialize(entry.getKey());
                		Integer fieldVal = Integer.parseInt(new String(entry.getValue()));
                		dataMap.put(fieldKey, fieldVal);
                	}
                }
                return dataMap;
            }
