...
public class ComPositionEao extends BaseHibernateEao<ComPosition, Integer> {
    ...
    * @throws
     ...
     * @return
     */
	public List<ComPosition> findComListByContact(Integer comId, Integer contactId) {
		String hql = "FROM ComPosition WHERE comId = ? and contactId = ?";
        if(contactId==null||contactId<0){//此时不按部门查找
            return null;
        }
		return list(hql, new Object[] {comId, contactId});
	}
}
