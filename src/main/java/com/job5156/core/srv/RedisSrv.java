...
public class RedisSrv {
     ...
     * @return
     */
    public Date getIndexReadTime(Integer perUserId, String type) {
        return new DateTime(redisEao.readFromString(Constants.REDIS_KEY_PER_INDEX_READ_TIME_PREFIX + type+":"+perUserId)).toDate();
    }
     ...
     * @param type
     */
    public void updateIndexReadTime(Integer perUserId, String type) {
        redisEao.writeToString(Constants.REDIS_KEY_PER_INDEX_READ_TIME_PREFIX + type+":"+perUserId, DateTime.now().toString());
        redisEao.setKeyExpire(Constants.REDIS_KEY_PER_INDEX_READ_TIME_PREFIX + type+":"+perUserId, Constants.REDIS_EXPIRE_TIME_DYNAMIC); //缓存的生存期
    }
}
