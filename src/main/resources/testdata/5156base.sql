...
CREATE TABLE `com_info` (
  ...
  `email_activation` tinyint(2) NOT NULL DEFAULT '0' COMMENT '邮件是否验证通过：0：为验证；1：已验证',
  `licence_check_stat` tinyint(2) NOT NULL DEFAULT '0' COMMENT '证照是否审核状态：0：未审核；1：审核',
  PRIMARY KEY (`id`),
...
CREATE TABLE `com_user` (
  ...
  `sms_own` int(10) NOT NULL DEFAULT '-1' COMMENT '可用最大点数-【发送短信】 【-1】为不限 ',
  `sms_used` int(10) NOT NULL DEFAULT '0' COMMENT '已用点数-【发送短信】',
  `pos_own` int(10) NOT NULL DEFAULT '-1' COMMENT '可用最大点数-【职位刷新】 【-1】为不限 ',
  `pos_used` int(10) NOT NULL DEFAULT '0' COMMENT '已用点数-【职位刷新】',
  PRIMARY KEY (`id`),
  KEY `IX_comId` (`com_id`),
  KEY `I_username` (`username`),
  KEY `I_deptid` (`dept_id`),
  KEY `I_roletype` (`role_type`)
